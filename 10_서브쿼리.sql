...
-- employee_id가 103번인 사람의 job_id 와 동일한 job_id 를 가진 사람을 조회.
...
-- David 라는 사람이 여러 명인데, 그 중에 가장 적은 급여를 받는 David 보다 급여가 높은 사람을 조회.
SELECT * FROM employees
...
-- David 라는 사람이 여러 명인데, 그 중에 가장 많은 급여를 받는 David 보다 급여가 높은 사람을 조회.
SELECT * FROM employees
WHERE salary > ALL (SELECT salary FROM employees
                WHERE first_name = 'David');

-- EXISTS: 서브쿼리가 하나 이상의 행을 반환하면 참으로 간주.
-- job_history 에 존재하는 직원이 employees 에도 존재한다면 조회대상에 포함.
-- 서브쿼리 내에서 jh에 있는 id 와 e에 있는 id가 일치할 때마다 1이라는 값을 조회.
-- EXISTS 연산자가 1이 조회가 될 떄 데이터가 존재한다는 것을 판단하여 employees 에서 해당사원의 모든 정보를 조회.
...
-- SELECT 절에 서브쿼리를 붙이기.
-- 스칼리 서브쿼리 라고도 칭합니다.
-- 스칼라 서브쿼리: 실행 결과가 단일 값을 반환하는 서브 쿼리. 주로 SELECT절이나, WHERE 절에서 사용됨.
