...
-- job id를 확인해서
-- 이미 존재하는 데이터라면 수정, 없다면 새롭게 추가 (job_id가 PK이기 떄문)
...
EXEC my_new_job_proc('JOB3', '테스트 job2', 10000, 12000);
...
-- OUT 을 이용해서 보낸 값은 바깥 익명 블록에서 실행해야 합니다.
CREATE OR REPLACE PROCEDURE my_new_job_proc
    (p_job_id IN jobs.job_id%TYPE,
     p_job_title IN jobs.job_title%TYPE,
     p_min_sal IN jobs.min_salary%TYPE := 0,
...
CREATE OR REPLACE PROCEDURE my_param_test_proc
    ...
    p_var1 IN VARCHAR2,
    -- OUT : 받는 용도로는 활용 불가능.
    -- OUT이 되는 시점은 프로시저가 끝날 떄. 그 전까지는 할당이 안됨.
    p_var2 OUT VARCHAR2,
    -- IN, OUT이 둘다 가능함.
