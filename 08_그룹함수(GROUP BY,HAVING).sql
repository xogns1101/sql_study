...
-- 주의할 점
-- 그룹 함수는 단독적으로 사용될 떄는 전체 테이블이 그룹의 대상이 되지만
...
-- GROUP BY 를 통해 그룹화 할떄 조건을 걸 경우 HAVING을 사용.
-- WHERE은 일반 조건절. GROUP BY 보다 먼저 진행됩니다.
SELECT
    department_id,
    AVG(salary)
FROM employees
...
ORDER BY 평균 DESC;
--FROM-WHERE-GROUP BY - HAVING -SELECT- ORDER BY 이 순서대로 돌아감
